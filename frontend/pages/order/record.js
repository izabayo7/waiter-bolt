            ...
            <div className="row justify-content-center">
                <div className={"col-8 shadow-lg rounded-sm " + styles.recordDiv}>
                    <div className="float-right">
                        {recordStatus == "RECORDED" ? <img src={cancelIcon.src} alt="cancel" className={styles.cancelIcon} onClick={() => setRecordStatus("NOT_RECORDED")} /> :recordStatus =="SENT" ? <button className={"btn px-3 py-2 "+globalStyles.globalTextColor+" "+styles.newOrderBtn} onClick={()=>{setRecordStatus("NOT_RECORDED")}}>New Order</button>: <></>}
                    </div>
                    <div className={"pl-5 " + styles.recordIcon}>
            
            {recordStatus == "SENT" ? <p className={"h5 mt-5 "+globalStyles.globalTextColor}>Your order will be ready in 10 minutes.</p> :<img src={recordStatus == "NOT_RECORDED" ? recordIcon.src : recordStatus =="LOADING" ? loadingRecordingIcon.src: sendIcon.src} alt="record" className="mt-5" onClick={() => setRecordStatus(recordStatus == "RECORDED" ? "LOADING" :recordStatus =="LOADING" ? "SENT":"RECORDED")} />}
